...
class Orchestrator:
    ...
    async def _execute_workflow(self, workflow_id: str) -> None:
        ...
        try:
            ...
            if workflow.status != WorkflowStatus.CANCELLED:
                if all(s.status == AgentStatus.COMPLETED for s in workflow.steps.values()):
    ...
    async def _wait_for_dependencies(self, workflow: Workflow, step: SpecStep) -> None:
        while True:
            deps_complete = all(
                workflow.steps.get(dep_id, StepExecution(step_id="")).status == AgentStatus.COMPLETED
                for dep_id in step.depends_on
            )
            if deps_complete:
                break
            any_failed = any(
                workflow.steps.get(dep_id, StepExecution(step_id="")).status == AgentStatus.FAILED
                for dep_id in step.depends_on
            )
            if any_failed:
